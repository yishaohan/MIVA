...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        print(f"        Initializing {model_class.__name__} structure on CPU...")
        init_device = torch.device("cpu")
        with init_weights_on_device(device=init_device):
            model = model_class(**extra_kwargs)
        if hasattr(model, "eval"):
            model = model.eval()
        missing_keys, unexpected_keys = model.load_state_dict(model_state_dict, assign=True, strict=False)
...
class ModelManager:
    ...
    def load_dit(self, dit_path, adapter_path=None, dit_kwargs:dict = {}):
        from diffsynth.models.wan_video_dit import WanModel
        dit = WanModel(**dit_kwargs)

        base_state_dict = load_state_dict(dit_path)
        missing_keys, unexpected_keys = dit.load_state_dict(state_dict=base_state_dict, strict=False)

        # initialize & load adapter parameters from the base model
        SA_mode = dit_kwargs.get("SA_mode", "vanilla")
        CA_mode = dit_kwargs.get("CA_mode", "vanilla")
        for name, param in dit.named_parameters():
            param.requires_grad = False # so that we can copy params
            # copy parameter weight from base model
            if SA_mode != "vanilla":
                if 'i2v_adapter.o' in name:
                    torch.nn.init.zeros_(param.data)
                elif 'i2v_adapter' in name and "weight_module" not in name:
                    source_w = name.replace('i2v_adapter', 'self_attn')
                    source_w = source_w.replace('q_m', 'q')
                    source_w = source_w.replace('o_m', 'o')
                    param.copy_(dit.state_dict()[source_w])
            if CA_mode != "vanilla":
                source_w = name.replace('sCA.q', 'cross_attn.q')
                source_w = name.replace('cross_attn_b.q', 'cross_attn.q')
                param.copy_(dit.state_dict()[source_w])
                pass
        ...
        if adapter_path:
            ...
            print("[Model_manager]Loaded ckpt:", adapter_path)

        self.model.append(dit)
        self.model_path.append(adapter_path)
        self.model_name.append("wan_video_dit")
